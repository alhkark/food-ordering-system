...
public class AIOrderNoteInterpreter implements OrderNoteInterpreter {
  ...
  @Qualifier("orderNoteInterpreterChatClient")
  private final ChatClient chatClient;

  @Value("classpath:/templates/order-interpreter-prompt.st")
  private final Resource orderNoteInterpreterPrompt;

  @Value("${order.ai.interpreter.max-attempts:3}")
  ...
  public OrderPreferences interpret(String orderNotes) {
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        lastException =
            new AIOrderNoteInterpreterException("Error in Order Interpreter AI response", e);
      }
    }
    throw lastException;
  }
  ...
  private OrderPreferences doInterpret(
      String orderNotes, PromptTemplate promptTemplate, String retryWarning) {
    Prompt prompt =
    ...
    return chatClient.prompt(prompt).system(SYSTEM_PROMPT).call().entity(OrderPreferences.class);
  }
}
